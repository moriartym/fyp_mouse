...
for epoch in range(300):
    ...
    if (epoch + 1) % 10 == 0:
        ...
        if acc > best_acc:
            best_acc   = acc
            best_epoch = epoch + 1
            torch.save(model.state_dict(), 'gesture_model_model_best.pt')

# Final report
print(f"\nBest val_acc={best_acc:.3f} at epoch {best_epoch}  (saved as gesture_model_best.pt)")

model.load_state_dict(torch.load('gesture_model_movement_best.pt'))
model.eval()
with torch.no_grad():
...
# save final
torch.save(model.state_dict(), 'gesture_model_movement.pt')
joblib.dump(le, 'label_encoder_movement.pkl')

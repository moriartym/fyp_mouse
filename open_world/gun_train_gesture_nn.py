...
# Load data
df = pd.read_csv('gun_gesture_data.csv')
...
joblib.dump(le, 'gun_label_encoder.pkl')
...
class GestureNet(nn.Module):
    def __init__(self, input_size, num_classes):
        ...
        self.net = nn.Sequential(
            nn.Linear(input_size, 256),  # 128 → 256
            nn.BatchNorm1d(256),
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(256, 128),         # new middle layer
            nn.BatchNorm1d(128),
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(128, 64),
...
for epoch in range(300):
    ...
    if (epoch + 1) % 10 == 0:
        ...
        if acc > best_acc:
            best_acc   = acc
            best_epoch = epoch + 1
            torch.save(model.state_dict(), 'gesture_model_best.pt')

# Final report
print(f"\nBest val_acc={best_acc:.3f} at epoch {best_epoch}  (saved as gesture_model_best.pt)")

model.load_state_dict(torch.load('gesture_model_gun_best.pt'))
model.eval()
with torch.no_grad():
...
# save final
torch.save(model.state_dict(), 'gesture_model_gun.pt')
joblib.dump(le, 'label_encoder_gun.pkl')
print("Done. gesture_model.pt + label_encoder.pkl ready.")
...
# ── must match train_gesture_nn.py exactly ─────────────────────────────────────
...
# ── must match collector exactly (scale-normalized) ────────────────────────────
...
while cap.isOpened():
    ...
    if result.hand_landmarks:
        ...
        final      = best_label if best_conf >= CONFIDENCE_THRESH else 'none'

        color = (0, 255, 0) if final != 'none' else (180, 180, 180)
